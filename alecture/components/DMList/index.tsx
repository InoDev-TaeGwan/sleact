...
const DMList:FC = () => {
    ...
    const {data: memberData} = useSWR<IUserWithOnline[]>( // workspace 에 참여한 member 데이터
                ...
                <div>
                    {!channelCollapse && 
                        memberData?.map((member) => {
                            ...
                            return(
                                <NavLink key={member.id} activeClassName="selected" to={`/workspace/dm/${member.id}`} onClick={resetCount(member.id)}>
                                    <i
                                    ...
                                    {
                                        member.id === userData?.id && <span>나</span> // 본인 일 경우 나 표시
                                    } 
